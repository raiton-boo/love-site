document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateMessage(distance, animate = true) {
        ...
        for (let i = 0; i < messageStages.length; i++) {
            if (distance >= messageStages[i].minDistance && distance <= messageStages[i].maxDistance) {
                newStageIndex = i;
                break;
            }
        }
    ...
    function drag(e) {
        ...
        if (dragTarget === leftBorder) {
            ...
            currentDistance = Math.max(minDist, Math.min(maxDist, currentDistance - (deltaX * 0.25)));
        ...
        startX = clientX;
        console.log('Dragging, distance:', currentDistance, 'min:', minDist, 'max:', maxDist); // デバッグ用
    }
